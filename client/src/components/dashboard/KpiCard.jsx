...
export function KpiCard({ label, value, delta, trend = "up", icon: Icon, accent = "green" }) {
  ...
  return (
    <Card className="rounded-[16px] p-5 shadow-soft transition hover:-translate-y-0.5 hover:shadow-lift">
      <div className="flex items-start justify-between gap-3">
        <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-ink-500">
          {label}
        </p>
        {Icon ? (
          <span className={cn("inline-flex h-9 w-9 items-center justify-center rounded-[10px]", accentClasses)}>
