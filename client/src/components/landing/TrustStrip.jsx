    ...
    <Stagger as="section" className="grid gap-4 lg:grid-cols-4" stagger={0.06}>
      {items.map(({ title, copy, icon: Icon }) => (
        <StaggerItem key={title}>
          <Card className="group h-full rounded-2xl p-5 transition-all duration-300 hover:-translate-y-1 hover:border-green-800 hover:shadow-glow">
            <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-green-100 text-green-800 transition-all duration-300 group-hover:bg-green-800 group-hover:text-white group-hover:shadow-md group-hover:shadow-green-800/20">
