...
const mapListing = (row = {}, options = {}) => {
  ...
  return {
    ...
    country: seller?.country || row.country || null,
...
const mapOrder = (row = {}, options = {}) => ({
  ...
  baseAmount: Number(row.base_amount || row.total_amount || 0),
  baseAmountLabel: formatCurrency(row.base_amount || row.total_amount || 0, row.currency || 'XAF'),
  amount: Number(row.total_amount || 0),
  amountLabel: formatCurrency(row.total_amount, row.currency || 'XAF'),
  logisticsRequired: Boolean(row.logistics_required),
  logisticsFee: Number(row.logistics_fee || 0),
  logisticsFeeLabel: formatCurrency(row.logistics_fee || 0, row.currency || 'XAF'),
  platformCommission: Number(row.platform_commission || 0),
  platformCommissionLabel: formatCurrency(row.platform_commission || 0, row.currency || 'XAF'),
  sellerNetAmount: Number(row.seller_net_amount || 0),
  sellerNetAmountLabel: formatCurrency(row.seller_net_amount || 0, row.currency || 'XAF'),
  currency: row.currency || 'XAF',
  status: row.status || 'pending_payment',
  buyerReceiptStatus: row.buyer_receipt_status || 'pending',
  buyerReceivedAt: row.buyer_received_at || null,
  payoutHoldReason: row.payout_hold_reason || null,
  notes: row.notes || '',
...
const mapPayment = (row = {}) => ({
  ...
  provider: row.metadata?.provider || 'internal_ledger',
  providerStatus: row.metadata?.providerStatus || null,
  checkoutUrl: row.metadata?.checkoutUrl || null,
